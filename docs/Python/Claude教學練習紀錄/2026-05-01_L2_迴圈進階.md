...
`input()` 取得的永遠是字串，`isdigit()` 用來確認使用者有沒有輸入合法的數字：

```python
'25'.isdigit()    # True  → 全是數字
'abc'.isdigit()   # False → 有字母
'12.5'.isdigit()  # False → 有小數點（點不算數字）
'-3'.isdigit()    # False → 有負號
''.isdigit()      # False → 空字串
```

確認是數字後才轉型，避免 `int('abc')` 報錯：

```python
text = input('輸入數字：')
if text.isdigit():
    num = int(text)   # ✅ 確認是數字才轉
```
