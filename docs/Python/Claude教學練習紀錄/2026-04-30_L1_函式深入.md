...
2 ** 3       # 8  （2 的 3 次方）
3 ** 2       # 9  （3 的 2 次方）
base ** exp  # base 的 exp 次方（變數版）
```

> `math.exp(x)` 是 e^x（自然指數），跟 `**` 不同，不要混淆。

---

### `print` vs `return` — 印出 vs 回傳

這兩個很容易混，題目說「**印出**」用 `print`，說「**回傳**」用 `return`。

```python
# print：把值印到畫面，函式本身回傳 None
def power_a(base, exp=2):
    print(base ** exp)

a = power_a(3)   # 畫面顯示 9
print(a)         # None ← 函式什麼都沒給出去

# return：把值交給呼叫者，呼叫者拿去用
...
**原則：** 函式的職責是「計算」，要不要印是呼叫者的事。用 `return` 讓函式可以被重複利用。驗證時在外面包 `print`：

```python
print(power_b(3))      # 驗證用，確認回傳值
print(power_b(2, 10))  # 1024
```
